package com.drms.disaster_relief.common.security;
...
public class JWTUtill {
    ...
    public String extractUserName(String token) {
        Claims claims = extractAllClaims(token);   // here we are using claims object because
...
/*
       ...
       Create Token:
        ...
        Validate Token:
                       ...
                       validateToken methods is called. it then calls isTokenExpired()  method to check if the
                       expiry date has passed or not. then it calls for another method extractExpiration. which
                        gets the expiry of token by calling the extractAllClaims method.
                        this method decodes the token. it first verifies the secret key of token by calling
                        getSigningKey method. if verifies then it decrypts the token and return the payload or body
                        if the token was changes it won't cross the verify secret key step.
                        the payload has loginIdentifier, issue date, expiry date, and role.
*/
