package com.drms.disaster_relief.mission.service;
...
import com.drms.disaster_relief.mission.dto.response.AssignLogisticsDto;
import com.drms.disaster_relief.mission.dto.request.CreateMissionDto;
import com.drms.disaster_relief.mission.dto.request.CrewAssignmentDto;
import com.drms.disaster_relief.mission.dto.response.AssignedCrewMemberDto;
import com.drms.disaster_relief.mission.dto.response.AssignedLogisticsSummaryDto;
import com.drms.disaster_relief.mission.dto.response.MissionDispatchResponseDto;
...
import com.drms.disaster_relief.mission.repo.ConsumableLogisticsAssignmentRepo;
import com.drms.disaster_relief.mission.repo.CrewAssignmentRepository;
import com.drms.disaster_relief.mission.repo.MissionRepository;
...
import com.drms.disaster_relief.mission.repo.ReturnableLogisticsAssignmentRepository;
...
public class MissionService {
    ...
    public MissionDispatchResponseDto crewAssignment(CrewAssignmentDto crewAssignmentDto){
       ...
       if(crewAssignmentDto.getDiversNeeded()>0)
           crewList.addAll(getAvailableCrew(EmployeeJob.DIVER, crewAssignmentDto.getDiversNeeded()));

       if(crewAssignmentDto.getDriversNeeded() > 0)
            crewList.addAll(getAvailableCrew(EmployeeJob.DRIVER, crewAssignmentDto.getDriversNeeded()));

       if(crewAssignmentDto.getFirefightersNeeded() > 0)
           crewList.addAll(getAvailableCrew(EmployeeJob.FIREFIGHTER, crewAssignmentDto.getFirefightersNeeded()));

       if(crewAssignmentDto.getSarSpecialistsNeeded() > 0)
           crewList.addAll(getAvailableCrew(EmployeeJob.SAR_SPECIALIST, crewAssignmentDto.getSarSpecialistsNeeded()));

       if(crewAssignmentDto.getMedicsNeeded() > 0)
           crewList.addAll(getAvailableCrew(EmployeeJob.MEDIC, crewAssignmentDto.getMedicsNeeded()));
    ...
    private List<Employee> getAvailableCrew(EmployeeJob name, int quantity) throws IllegalStateException {
    ...
    @Transactional
    public AssignedLogisticsSummaryDto assignLogistics(AssignLogisticsDto dto) {
        ...
        AssignedLogisticsSummaryDto response = new AssignedLogisticsSummaryDto();
        response.setMissionId(mission.getMissionId());
        response.setMissionName(mission.getMissionName());
